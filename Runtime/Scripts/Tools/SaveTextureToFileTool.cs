...
public static class SaveTextureToFileTool
{
   ...
   public static void SaveTextureToFile(Texture source,
                                         ...
                                         System.Action<bool> done = null)
    {
        ...
        var request = AsyncGPUReadback.RequestIntoNativeArray (ref nativeArray, resizeRT, 0, (AsyncGPUReadbackRequest request) =>
        {
            ...
            if (!request.hasError)
            {
                ...
                switch (fileFormat)
                {
                    case SaveTextureFileFormat.EXR:
                        encoded = ImageConversion.EncodeNativeArrayToEXR(nativeArray, resizeRT.graphicsFormat, (uint)width, (uint)height);
                        break;
                    case SaveTextureFileFormat.JPG:
                        encoded = ImageConversion.EncodeNativeArrayToJPG(nativeArray, resizeRT.graphicsFormat, (uint)width, (uint)height, 0, jpgQuality);
                        break;
                    case SaveTextureFileFormat.TGA:
                        encoded = ImageConversion.EncodeNativeArrayToTGA(nativeArray, resizeRT.graphicsFormat, (uint)width, (uint)height);
                        break;
                    default:
                        encoded = ImageConversion.EncodeNativeArrayToPNG(nativeArray, resizeRT.graphicsFormat, (uint)width, (uint)height);
                        break;
                }
